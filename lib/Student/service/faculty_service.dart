...
import '../models/faculty_model.dart'; // Add this import
...
class FacultyService {
  ...
  /// 👨‍🏫 Fetch all faculty/teachers
  Future<List<FacultyModel>> fetchAllFaculty(String authToken) async {
    final url = Uri.parse('${ApiConfig.baseUrl}/api/students/my/batchFaculty'); // Update endpoint as needed
    print('📘 [ApiService] Fetching faculty from $url');
    ...
    try {
      ...
      print('📩 Faculty Response Status: ${response.statusCode}');
      if (response.statusCode == 200) {
        List<dynamic> jsonResponse = json.decode(response.body);
        print('✅ Faculty fetched successfully, count: ${jsonResponse.length}');
        return jsonResponse.map((data) => FacultyModel.fromJson(data)).toList();
      } else {
        print('❌ Faculty fetch failed: ${response.body}');
        throw Exception(
            'Failed to load faculty. Status Code: ${response.statusCode}');
      }
    } catch (e) {
      print('⚠️ Error in fetchAllFaculty(): $e');
      throw Exception('An error occurred while fetching faculty: $e');
    }
  }
}