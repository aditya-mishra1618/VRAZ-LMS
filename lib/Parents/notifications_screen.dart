...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  final UniversalNotificationService _notificationService = UniversalNotificationService.instance;
  ...
  Future<void> _initializeNotifications() async {
    ...
    try {
      // 1. Load parent credentials from SharedPreferences
      final prefs = await SharedPreferences.getInstance();
      _authToken = prefs.getString('parent_auth_token');
      _parentPhone = prefs.getString('parent_phone_number');

      print('[ParentNotif] Auth Token: ${_authToken != null ? "Found" : "Missing"}');
      print('[ParentNotif] Parent Phone: $_parentPhone');

      // ✅ Set role to 'parent' for isolated storage
      _notificationService.setRole('parent');

      // 2. Initialize the notification service
      await _notificationService.initialize();

      // 3. Fetch notifications from server (NO registration here!)
      print('[ParentNotif] 🔄 Fetching notifications from server...');
      ...
      print('[ParentNotif] 📥 Loaded ${_notifications.length} PARENT notifications');

      // 5. Listen for real-time notification updates
      _notificationSubscription = _notificationService.notificationsStream.listen((updatedList) {
        if (mounted) {
          print('[ParentNotif] 🔔 Real-time update: ${updatedList.length} notifications');
          setState(() {
      ...
      _isInitialized = true;
      print('[ParentNotif] ✅ Initialized with ${_notifications.length} notifications');
    } catch (e, stackTrace) {
      print('[ParentNotif] ❌ Initialization error: $e');
      print('[ParentNotif] Stack trace: $stackTrace');
      if (mounted) {
  ...
  Future<void> _refreshNotifications() async {
    setState(() => _isLoading = true);
    try {
      print('[ParentNotif] 🔄 Manual refresh triggered...');
      await _notificationService.fetchAndMergeFromServer(authToken: _authToken);
      setState(() {
        _notifications = _notificationService.getStoredNotifications();
      });
      print('[ParentNotif] ✅ Refresh complete: ${_notifications.length} notifications');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
      ...
      print('[ParentNotif] ❌ Refresh error: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
  ...
  Future<void> _markAllAsRead() async {
    print('[ParentNotif] 📖 Marking all notifications as read...');
    ...
    print('[ParentNotif] ✅ All notifications marked as read');
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
  ...
  void _showNotificationDetailsDialog(
    ...
    print('[ParentNotif] 📖 Marking notification as read: ${notification.id}');
