...
class TeacherTimetableService {
  final String baseUrl = 'http://vraz-backend-api.onrender.com/api/teachers';
  final String token;
  ...
  Future<List<TeacherTimetableEntry>> fetchTimetable({
    ...
    final url = Uri.parse('$baseUrl/my/schedule?startDate=$startDate&endDate=$endDate');
    ...
    print('[DEBUG] Cleaned token: [$cleanToken] (length: ${cleanToken.length})');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print('[DEBUG] Timetable fetched successfully: ${data.length} entries.');
        ...
        print('[ERROR] Failed to fetch timetable. Status code: ${response.statusCode}');
        throw Exception('Failed to fetch timetable. Status code: ${response.statusCode}');
      }
    } catch (e) {
